...
var <%= solutionName %> = require("./scripts/solution.js");
...
gulp.task("01-Copy-Sitecore-Lib", function () {
  ...
  var files = path.join(config.sitecoreLibraries, "/**/*");
  var excludeFiles = "!" + path.join(config.sitecoreLibraries, "/**/") + config.solutionName + "*";
    
  return gulp.src([files,excludeFiles]).pipe(gulp.dest("./lib/Sitecore"));
});
...
gulp.task("05-Sync-Unicorn", function (callback) {
  var options = {};
  options.siteHostName = <%= solutionName %>.getSiteUrl();
...
gulp.task("Publish-Assemblies", function () {
  var root = "./src";
  var binFiles = root + "/**/code/**/bin/<%= solutionName %>.{Feature,Foundation,Website}.*.{dll,pdb}";
...
gulp.task("Auto-Publish-Assemblies", function () {
  ...
  var files = "/**/<%= solutionName %>.{Feature,Foundation,Website}.*.{dll,pdb}";;
  var destination = config.websiteRoot + "/bin/";
  gulp.src(roots, { base: root }).pipe(
    foreach(function (stream, rootFolder) {
      gulp.watch(rootFolder.path + files, function (event) {
